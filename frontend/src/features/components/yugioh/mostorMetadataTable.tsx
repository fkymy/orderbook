        ...
        {['属性', 'レベル', '攻撃力', '防御力'].map((colName, idx) => {
          return (
            <Box
              ...
              key={idx}
            >
              <Center height='32px'>{colName}</Center>
            </Box>
          )
        })}
      </Grid>
      <Grid height='42px' templateColumns='repeat(4, 1fr)'>
        <Box borderWidth='0px 1px 1px 1px' borderColor='#8F8F8F'>
        ...
        <Box borderWidth='0px 1px 1px 0px' borderColor='#8F8F8F'>
          <Flex justify='center' alignItems='center' height='42px'>
            <Spacer />
            <Spacer />
            <Box>
              <Center>
                <NextImage src={LevelIcon} width='24px' height='24px' />
              </Center>
            </Box>
            <Spacer />
            <Text as='b' fontSize='20'>
